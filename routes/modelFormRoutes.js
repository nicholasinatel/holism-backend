    ...
    return {
        ...
        validate: {
          ...
          payload: {
            title: Joi.string()
              ...
              .min(3)
              .max(100),
            step_forward: Joi.array()
              .min(1)
              .items(Joi.string())
              .default(['ffffffffffffffffffffffff']),
            step_backward: Joi.array()
              .min(1)
              .items(Joi.string())
      ...
      handler: async request => {
        try {
          ...
          let { tempoUtilizado } = request.payload;
    ...
    return {
      ...
      handler: async request => {
        try {
           ...
           */
          if (
            nuId.step_backward[0] !== '000000000000000000000000' &&
            nuId.step_forward[0] === 'ffffffffffffffffffffffff'
          ) {
            /**
             * * Last Form Condition
          ...
          if (
            ...
            await this.dbFlow.update(nuId.flow, {
              starter_form: nuId.step_forward
            });
          }
          if (
            nuId.step_forward[0] === 'ffffffffffffffffffffffff' &&
          ...
          if (
             ...
             */
            if (nuId.status !== 3) {
              const [formAnterior] = await this.db.read({
                _id: nuId.step_backward
              });

              const [formPosterior] = await this.db.read({
                _id: nuId.step_forward
              });

              if (formAnterior.status === 3 && formPosterior.status === 0) {
                turnStatus = true;
              }
            }
